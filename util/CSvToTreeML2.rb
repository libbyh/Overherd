...
#  puts data_array
  ...
  branch2.add_element("attribute","name"=>"message_date", "value"=>data_array[6])
...
# supported date types for TreeML are: Integer, Long, Float, Real, String, Date
doc=Document.new
...
doc.root.elements["declarations"].add_element("attributeDecl", {"name"=>"forum_date","type"=>"Date"})
...
doc.root.elements["declarations"].add_element("attributeDecl", {"name"=>"topic_date","type"=>"Date"})
...
doc.root.elements["declarations"].add_element("attributeDecl", {"name"=>"message_date","type"=>"Date"})
...
#experimental - big hash has a node id and link to dom tree, for a fast lookup
big_hash=Hash.new
...
forums_array.each{|forum|
 ...
 branch2.add_element("attribute","name"=>"forum_date", "value"=>forum[3])
...
# puts temp_extended
 branch2.add_element("attribute","name"=>"forum_extended", "value"=>forum[6])
  ...
  #add it to big_hash so its childrent can have a fast lookup
  big_hash[forum[1]]=branch2
  puts "forum id: " + forum[1]
  puts big_hash[forum[1]]
}
...
topics_array.each {|topic|
  ...
  branch4.add_element("attribute","name"=>"topic_date","value"=>topic[4])
...
messages_array.each{ |message|
 ...
 #   branch2.add_element("attribute","name"=>"message_body","value"=>message[11])
    ...
    temp_not_inserted=Array.new(not_inserted.keys)
   ...
   # if(!added.include?(reply_to))
      ...
      if(!not_inserted.has_key?(countline))
        not_inserted[countline]=branch
  ...
  #    branch2.add_element("attribute","name"=>"message_body","value"=>message[11])
    ...
    num_to_check=not_inserted.keys.length
    puts "not_inserted len: " + not_inserted.keys.length.to_s()
    checked_count=0
...
checkcount=5
while(not_inserted.keys.length>0 and checkcount>0)
